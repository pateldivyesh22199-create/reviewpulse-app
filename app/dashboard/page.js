...
import { 
  ...
  Save, Smartphone, MessageSquare, Menu, X, Wand2, TrendingUp, BarChart3, 
  Search, CheckCircle2, PlusCircle, User, Printer, RefreshCcw, Power, ChevronDown 
} from "lucide-react";
...
export default function Dashboard() {
    ...
    async function loadData() {
      try {
        ...
        if (data.businesses && data.businesses.length > 0) {
          ...
          const current = data.businesses[0];
          setSelectedBiz(current);
          setStats({
            id: current.id, creditsUsed: data.credits_used || 0, plan: data.plan || "free",
            maxLocations: data.max_locations || 1, googleLink: current.google_link || "",
  ...
  const handleGoogleSearch = () => {
    ...
    const magicLink = `https://www.google.com/search?q=${searchQuery.replace(/\s+/g, '+')}+reviews`;
  ...
  const NavItems = () => (
    <nav className="flex-1 px-6 space-y-3 mt-4">
      ...
      <button onClick={() => {setActiveTab("history"); setIsMobileMenuOpen(false);}} className={`w-full flex items-center gap-3 px-5 py-4 rounded-2xl transition-all ${activeTab === "history" ? "bg-blue-600 text-white shadow-xl" : "text-slate-500 hover:bg-white/5"}`}><History size={20} /> <span className="text-sm font-bold">History</span></button>
      ...
      <button onClick={() => window.location.href='/dashboard/account'} className="w-full flex items-center gap-3 px-5 py-4 rounded-2xl text-slate-500 hover:bg-white/5"><User size={20} /> <span className="text-sm font-bold">Account</span></button>
    </nav>
  );
    ...
    <div className="flex min-h-screen bg-[#030712] text-slate-200">
      <aside className="w-72 border-r border-white/5 bg-[#030712] hidden md:flex flex-col sticky top-0 h-screen z-50">
        ...
        <div className="p-6 mt-auto">
            <div className="p-5 rounded-[2rem] bg-slate-900 border border-white/5 text-center shadow-inner">
                <p className="text-[10px] uppercase text-slate-500 font-black tracking-widest">Plan: {stats.plan}</p>
                <button onClick={() => window.location.href='/dashboard/account'} className="w-full mt-4 bg-white text-black text-[10px] font-black py-3 rounded-xl uppercase">Manage Billing</button>
            </div>
        </div>
      </aside>

      {isMobileMenuOpen && (
        <div className="fixed inset-0 bg-[#030712] z-[100] flex flex-col md:hidden">
          ...
          <div className="flex items-center gap-4">
             ...
             <div className="relative group">
                <button className="flex items-center gap-3 bg-white/5 border border-white/5 px-4 py-2 rounded-xl hover:border-blue-500/50 transition-all">
                  ...
                  <span className="text-xs md:text-sm font-black text-white uppercase tracking-widest truncate max-w-[150px]">{selectedBiz?.name || "Loading..."}</span>
                ...
                <div className="absolute top-full left-0 mt-2 w-64 bg-slate-900 border border-white/10 rounded-2xl shadow-3xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 p-2">
                   {businesses.map((biz) => (
                     <button key={biz.id} onClick={() => switchBusiness(biz)} className="w-full text-left px-4 py-3 rounded-xl hover:bg-blue-600 hover:text-white transition-colors flex items-center justify-between text-[10px] font-black uppercase">
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
                 <div className="lg:col-span-2 bg-slate-900/40 border border-white/5 p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden">
                    <div className="flex justify-between items-center mb-8"><div><h3 className="text-lg font-bold text-white flex items-center gap-2"><TrendingUp size={20} className="text-emerald-400" /> Intelligence Monitoring</h3><p className="text-xs text-slate-500">Metrics for {selectedBiz?.name}</p></div></div>
                 ...
                 <div className="space-y-6">
                    <div className="bg-slate-900/40 border border-white/5 p-6 rounded-[2rem]"><div className="flex items-center gap-4"><div className="p-3 bg-blue-500/10 rounded-2xl text-blue-500"><Clock size={20} /></div><div><p className="text-[10px] font-black text-slate-500 uppercase">Hours Saved</p><h3 className="text-xl font-black text-white">{stats.timeSaved} Hrs</h3></div></div></div>
                    ...
                    <div className="bg-blue-600 p-6 rounded-[2rem] shadow-2xl"><p className="text-[10px] font-bold text-blue-100 uppercase">System Credits</p><h3 className="text-2xl font-black text-white mt-1">{stats.plan === 'free' ? (10 - stats.creditsUsed) : '∞ Unlimited'}</h3></div>
                 </div>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                <div className="bg-slate-900/60 border border-white/5 rounded-[2.5rem] p-6 md:p-10 shadow-3xl">
                  <h3 className="text-lg font-bold text-white mb-8 flex items-center gap-3"><MessageSquare className="text-blue-500" size={20} /> New Interaction</h3>
                  <div className="space-y-6">
                    ...
                    <button onClick={handleGenerate} disabled={loading} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-5 rounded-[1.5rem] transition-all disabled:opacity-50 uppercase tracking-widest text-xs flex items-center justify-center gap-3">{loading ? "Agent Thinking..." : <><Zap size={16} /> Deploy Intelligence</>}</button>
                  </div>
                </div>
                <div className="bg-slate-900/60 border border-white/5 rounded-[2.5rem] p-6 md:p-10 shadow-3xl flex flex-col relative overflow-hidden"><div className="absolute top-0 right-0 w-32 h-32 bg-blue-500/5 rounded-full blur-3xl"></div><h3 className="text-lg font-bold text-white mb-8">AI Response Output</h3><div className="flex-1 bg-black/40 border border-white/5 rounded-[1.5rem] p-6 md:p-8 relative">{aiResponse ? ( <p className="text-slate-300 text-sm leading-relaxed italic">"{aiResponse}"</p> ) : ( <div className="h-full flex flex-col items-center justify-center text-slate-700 opacity-20"><Zap size={48} /><p className="text-[10px] font-black uppercase mt-4 tracking-widest">System Ready</p></div> )}</div>{aiResponse && ( <button onClick={() => {navigator.clipboard.writeText(aiResponse); setCopied(true); setTimeout(()=>setCopied(false),2000)}} className="mt-8 flex items-center justify-center gap-3 bg-slate-800 hover:bg-slate-700 text-white font-black py-4 rounded-2xl transition-all text-xs uppercase">{copied ? <Check size={18} className="text-emerald-400" /> : <Copy size={18} />} Copy Intelligence</button> )}</div>
              </div>
            </div>
          )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                   <div className="lg:col-span-2 space-y-6">
                      <div className="bg-slate-900/40 border border-white/5 p-6 md:p-10 rounded-[3rem] shadow-2xl">
                         ...
                         <div className="relative group mb-4"><Search className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-500" size={18} /><input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search business name in Canada/USA..." className="w-full bg-slate-950 border border-white/5 rounded-2xl pl-14 pr-32 py-5 text-sm outline-none focus:border-blue-500 transition-all" /><button onClick={handleGoogleSearch} className="absolute right-3 top-1/2 -translate-y-1/2 bg-blue-600 text-white px-4 py-2 rounded-xl text-[9px] font-black uppercase">Find</button></div>
                         <div className="flex gap-3 pt-6 border-t border-white/5"><input type="text" value={stats.googleLink} onChange={(e) => setStats({...stats, googleLink: e.target.value})} placeholder="Linked Google Review URL" className="flex-1 bg-slate-950/50 border border-white/5 rounded-2xl px-5 py-4 text-xs outline-none italic text-slate-500" /><button onClick={saveGoogleLink} className="bg-blue-600 px-6 rounded-2xl transition-all"><Save size={20} /></button></div>
                      </div>
                      <div className="bg-blue-600/5 border border-blue-500/10 p-6 md:p-8 rounded-[2.5rem] flex items-center gap-6"><ShieldCheck className="text-blue-500" size={32} /><p className="text-xs text-slate-400">Crisis Shield™ is Active for {selectedBiz?.name}. public ratings are 100% protected.</p></div>
                   </div>
                   <div className="bg-slate-900/40 border border-white/5 p-8 md:p-10 rounded-[3rem] flex flex-col items-center">
                      <div className="bg-white p-4 md:p-6 rounded-[2.5rem] shadow-3xl border-[10px] border-white">{selectedBiz?.id ? <img src={`https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(reviewLink)}`} alt="QR" className="w-40 h-40" /> : "Syncing..."}</div>
                      <p className="mt-8 text-sm font-black text-center">{selectedBiz?.name}</p>
                      <button onClick={() => window.open(reviewLink)} className="mt-10 w-full bg-slate-800 hover:bg-slate-700 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all">Preview Experience</button>
                   </div>
                </div>
             </div>
          )}
          ...
          {activeTab === "history" && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-500">
               <div className="bg-slate-900/40 border border-white/5 rounded-[2.5rem] overflow-hidden shadow-2xl">
                  <div className="p-6 md:p-10 border-b border-white/5 bg-slate-900/20 flex justify-between items-center"><h3 className="text-xl md:text-2xl font-black uppercase tracking-widest">Intelligence Audit Logs</h3></div>
                  <div className="overflow-x-auto">
                     <table className="w-full text-left">
                        <tbody className="divide-y divide-white/5">
                           {history.map((rev, idx) => (
                              <tr key={idx} className="hover:bg-white/[0.02] transition-colors">
                                 <td className="px-6 md:px-10 py-6"><div className="flex items-center gap-3"><div className={`w-2 h-2 rounded-full ${rev.sentiment === 'negative' ? 'bg-red-500 animate-pulse' : 'bg-emerald-500'}`}></div><div><p className="text-sm font-bold text-slate-300">{rev.reviewer_name}</p><div className="flex text-amber-500 mt-1">{Array.from({length: rev.rating}).map((_, i) => <Star key={i} size={10} className="fill-current" />)}</div></div></div></td>
